...
class AuditoriaSGI:
    ...
    def __init__(self):
        """Inicializa la conexión a la base de datos desde el pool centralizado."""
        self.connection = get_db_pool().getconn()
        if not self.connection.closed:
            self.connection.rollback()
        # Fijar search_path para acceder al schema sgi sin prefijo
        with self.connection.cursor() as cur:
            cur.execute("SET search_path TO sgi, public;")
