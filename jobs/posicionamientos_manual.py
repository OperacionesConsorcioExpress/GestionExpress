# ============================================================
#  JOB POSICIONAMIENTOS (MANUAL) - PROCESA FECHAS ESPECÍFICAS
#  Copia de posicionamientos.py para reprocesar fechas puntuales
#  sin depender de la última fecha registrada en el log.
# ============================================================
import os, sys, io, re, math, time, traceback
...
# Fechas puntuales a reprocesar manualmente
FECHAS_MANUALES = [
    date(2026, 8, 6),
    date(2026, 8, 6),
]
...
# ============================================================
# 7) ORQUESTADOR MANUAL: PROCESA SOLO LAS FECHAS INDICADAS
# ============================================================
def ejecutar_job_posicionamientos_manual(fechas: list[date], verbose: bool = False):
    """
    Reprocesa manualmente una lista puntual de fechas, sin importar
    cuál sea la última fecha registrada en log.procesa_posicionamientos.
    Útil para reprocesar días específicos que quedaron pendientes o
    con error, aunque ya existan fechas posteriores procesadas.
    """
    print(f"📅 Fechas a procesar manualmente: {[f.strftime('%Y-%m-%d') for f in fechas]}")
    print()
    ...
    for i, dia in enumerate(fechas, 1):
        print(f"\n[{i}/{len(fechas)}] Procesando día: {dia} ...")
        t0 = time.time()
...
if __name__ == "__main__":
    ...
    ejecutar_job_posicionamientos_manual(FECHAS_MANUALES, verbose=verbose)
