...
DEFAULT_ID_REPORTE = 6
NOMBRE_REPORTE_LOG = "Tabla Asignaciones"
...
class AsignacionesBuilder:
    ...
    def load_detallado(self, fecha: datetime) -> pd.DataFrame:
        ...
        if c_vehiculo_real is None:
            raise SystemExit("❌ No se encontró 'Vehículo Real' en Detallado, necesario para cruce de Asignaciones.")
    ...
    def load_asignaciones(self, fecha: datetime) -> pd.DataFrame:
        print("\n" + "=" * 80)
        print("3) CARGANDO ASIGNACIONES (AZURE)")
        print("=" * 80)
        ...
        if not frames:
            raise SystemExit("❌ No se encontraron archivos de Asignaciones para esa fecha.")

        df = pd.concat(frames, ignore_index=True, sort=False)
        df = self.io.limpiar_columnas(df)

        print("📋 Columnas detectadas en Asignaciones:")
        print(list(df.columns))

        c_apply_date = pick_col(df, ["APPLY_DATE", "Apply Date"])
        c_veh_serv_id = pick_col(df, ["VEH_SERV_ID", "Veh Serv Id"])
        c_veh_registr_num = pick_col(df, ["VEH_REGISTR_NUM", "Veh Registr Num"])
        c_start_datetime = pick_col(df, ["START_DATETIME", "Start Datetime"])
        c_end_datetime = pick_col(df, ["END_DATETIME", "End Datetime"])
        c_assign_stat_type_cd = pick_col(df, ["ASSIGN_STAT_TYPE_CD", "Assign Stat Type Cd"])
        c_assign_type_cd = pick_col(df, ["ASSIGN_TYPE_CD", "Assign Type Cd"])
        ...
        dt_start = self.tu.to_datetime_yyyymmdd_hhmmss(out[c_start_datetime])
        dt_end = self.tu.to_datetime_yyyymmdd_hhmmss(out[c_end_datetime])

        out["HoraAsig"] = dt_start
        out["HoraFin"] = dt_end
        out["Status"] = out[c_assign_stat_type_cd].astype("string").str.strip()
    ...
    def build(self, fecha: datetime) -> tuple[pd.DataFrame, str]:
        ...
        fecha_nombre = self.tu.fecha_para_nombre_archivo_dd_mm_yyyy(df_asig["Fecha"].astype(str))
        ...
        out = pd.DataFrame({
            ...
            "HoraAsig": df_merge["HoraAsig"],
            "HoraFin": df_merge["HoraFin"],
            "Status": df_merge["Status"],
            "StatusIni": df_merge["StatusIni"],
        })
        ...
        out = out.drop_duplicates(
            subset=[
                "Id_ICS", "Fecha", "Servicio", "Vehiculo",
                "HoraAsig", "HoraFin", "Status", "StatusIni"
            ],
            keep="first"
        ).reset_index(drop=True)

        print("\n✅ Tabla Asignaciones construida:")
        print(f"   Filas con Id_ICS: {len(out)}")
        print(f"📌 Nombre lógico: ASIGNACIONES_{fecha_nombre}")

        return out, fecha_nombre

# =============================================================================
# POSTGRES LOAD sne.asignaciones
# =============================================================================

class PostgresAsignacionesLoader:
    DF_TO_DB = {
        "Id_ICS": "id_ics",
        "Fecha": "fecha",
        "Servicio": "servicio",
    ...
    def _prepare_df_for_db(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        d["horaasig"] = d["horaasig"].apply(self._parse_datetime)
        d["horafin"] = d["horafin"].apply(self._parse_datetime)
        d["id_ics"] = pd.to_numeric(d["id_ics"], errors="coerce")

        for c in ["servicio", "vehiculo", "status", "statusini"]:
    ...
    def insert_df(self, df: pd.DataFrame) -> int:
        ...
        sql = f"""
            ...
            DO UPDATE SET
                "horaasig" = EXCLUDED."horaasig",
                "horafin" = EXCLUDED."horafin",
                "status" = EXCLUDED."status",
                "statusini" = EXCLUDED."statusini"
        """
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                ...
                for start in range(0, len(df_db), self.batch_size):
                    ...
                    total += len(records)
                    print(f"  ✅ Upsert asignaciones {total}/{len(df_db)}")
...
def _export_df_to_csv(df: pd.DataFrame, fecha_nombre: str) -> Optional[str]:
    ...
    filename = f"ASIGNACIONES_{fecha_nombre}.csv"
...
def main() -> None:
    ...
    try:
        ...
        loader = PostgresAsignacionesLoader(
