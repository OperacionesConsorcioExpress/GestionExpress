...
AZURE_CONTAINER_ACCIONES_REG = "e01-fms"
...
class AccionesRegulacionBuilder:
    def __init__(
        self,
        azure_reader: AzureBlobReader,
        acciones_reader: AzureBlobReader,
        filtro_zona_tipo: int,
    ):
        self.az = azure_reader
        self.az_acciones = acciones_reader
        self.io = DataIO()
    ...
    def _blob_paths_acciones_reg(self, fecha: datetime) -> List[Tuple[str, str]]:
        ...
        for tipo in self._subpaths():
            for zona_corta, zona_blob in [("SC", "sc"), ("UQ", "uq")]:
                raiz = "1_sc" if zona_blob == "sc" else "2_uq"
                if tipo == "ZN":
                    carpeta = f"90_acciones_regulacion_zonal_{zona_blob}"
                    nombre = f"{fecha_txt}_acciones_regulacion_zonal_{zona_blob}.csv"
                else:
                    carpeta = f"91_acciones_regulacion_troncal_{zona_blob}"
                    nombre = f"{fecha_txt}_acciones_regulacion_troncal_al_{zona_blob}.csv"
    ...
    def load_acciones_reg(self, fecha: datetime) -> pd.DataFrame:
        ...
        for ruta, nombre in self._blob_paths_acciones_reg(fecha):
            ...
            df0 = self.io.leer_csv_desde_bytes(raw, dtype=str)
...
def main() -> None:
    ...
    try:
        ...
        az_acciones = AzureBlobReader(
            AzureConfig(
                connection_string=conn_azure,
                container_actividad=AZURE_CONTAINER_ACCIONES_REG,
            )
        )
        builder = AccionesRegulacionBuilder(
            az,
            az_acciones,
            filtro_zona_tipo=FILTRO_ZONA_TIPO,
        )
