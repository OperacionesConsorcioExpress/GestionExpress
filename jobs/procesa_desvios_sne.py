...
PG_BATCH_SIZE = 5000
NOMBRE_REPORTE_LOG = "Tabla Desvios"
DEFAULT_ID_REPORTE = 7
...
class DesviosBuilder:
    def __init__(self, azure_reader: AzureBlobReader, fms_reader: AzureBlobReader):
    ...
    def _blob_paths_desvios(self, fecha: datetime) -> List[Tuple[str, str]]:
        anio = fecha.strftime("%Y")
        fecha_txt = fecha.strftime("%Y%m%d")
        ...
        for zona_blob in ["sc", "uq"]:
            ...
            carpeta = f"10_TBFCD103_desvios_zonal_{zona_blob}"
            nombre = f"{fecha_txt}_line_driving_control_zonal_{zona_blob}.csv"
    ...
    def load_desvios(self, fecha: datetime) -> pd.DataFrame:
        print("\n" + "=" * 80)
        print("2) CARGANDO DESVIOS (AZURE)")
        print("=" * 80)

        frames: List[pd.DataFrame] = []
        for ruta, nombre in self._blob_paths_desvios(fecha):
        ...
        if not frames:
            raise SystemExit("❌ No se encontraron archivos de Desvios para esa fecha.")

        df = pd.concat(frames, ignore_index=True, sort=False)
        df = self.io.limpiar_columnas(df)

        print("📋 Columnas detectadas en Desvios:")
        ...
        c_serv_trip_seq = pick_col(df, ["SERV_TRIP_SEQ", "Serv Trip Seq", "TRIP_SEQ", "TRIP_ID"])
        ...
        c_real_detur_dist = pick_col(df, ["REAL_DETUR_DIST", "Real Detur Dist"])
        ...
        out["IdViaje"] = out[c_serv_trip_seq].astype("string").str.strip()
        ...
        out["DstMuestreo"] = self.tu.to_numeric(out[c_real_detur_dist])
    ...
    def build(self, fecha: datetime) -> tuple[pd.DataFrame, str]:
        ...
        out = pd.DataFrame({
            ...
            "DstMuestreo": df_merge["DstMuestreo"],
        })
        ...
        out = out.drop_duplicates(
            subset=[
                "Id_ICS", "Fecha", "Servicio", "IdViaje", "Motivo",
                "Linea", "Vehiculo", "OffsetIni", "OffsetFin",
                "DstEstimada", "DstMuestreo"
            ],
            keep="first"
        ).reset_index(drop=True)

        print("\n✅ Tabla Desvios construida:")
        print(f"   Filas con Id_ICS: {len(out)}")
        print(f"📌 Nombre lógico: DESVIOS_{fecha_nombre}")

        return out, fecha_nombre

# =============================================================================
# POSTGRES LOAD sne.desvios
# =============================================================================

class PostgresDesviosLoader:
    DF_TO_DB = {
        "Id_ICS": "id_ics",
        "Fecha": "fecha",
        "Servicio": "servicio",
    ...
    def _prepare_df_for_db(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        d["offsetini"] = pd.to_numeric(d["offsetini"], errors="coerce")
        d["offsetfin"] = pd.to_numeric(d["offsetfin"], errors="coerce")
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                ...
                for start in range(0, len(df_db), self.batch_size):
                    ...
                    total += len(records)
                    print(f"  ✅ Upsert desvios {total}/{len(df_db)}")
...
class ReportRunLogger:
    ...
    def get_id_reporte(self, nombre_reporte: str, default_id: int = 7) -> int:
        sql = f"""
            SELECT id_reporte
...
def _export_df_to_csv(df: pd.DataFrame, fecha_nombre: str) -> Optional[str]:
    ...
    filename = f"DESVIOS_{fecha_nombre}.csv"
...
def main() -> None:
    ...
    try:
        ...
        builder = DesviosBuilder(az, az_fms)
        ...
        print("\n" + "=" * 80)
        print("4) CARGANDO A POSTGRES sne.desvios")
        print("=" * 80)

        loader = PostgresDesviosLoader(
                        schema=PG_SCHEMA_NAME,
            table=PG_TABLE_NAME,
            batch_size=PG_BATCH_SIZE
        )
        total = loader.insert_df(df_final)
        print(f"✅ sne.desvios upsert: {total} filas")
        ...
        if fecha_dt.date() == fecha_limite and ("no existe ics en azure" in str(e).lower() or "no se encontraron archivos de desvios" in str(e).lower()):
        ...
        try:
            ...
            id_reporte = logger.get_id_reporte("Tabla Desvios", default_id=7)
    ...
    id_reporte_tail = logger_tail.get_id_reporte("Tabla Desvios", default_id=7)
